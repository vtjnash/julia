                ...
                if isdefined(code, :inferred)
                    if code.jlcall_api == 2
                        if needtree
                            ...
                            tree.slotnames = Any[compiler_temp_sym]
                            tree.slotflags = UInt8[]
...
function finish(me::InferenceState)
    ...
    if me.optimize
        ...
        void_use_elim_pass!(me)
        meta_elim_pass!(me.src.code::Array{Any,1}, me.src.propagate_inbounds)
...
function inlineable(f::ANY, ft::ANY, e::Expr, atypes::Vector{Any}, sv::InferenceState)
    ...
    if !isempty(stmts)
        if all(stmt -> (isa(stmt, Expr) && is_meta_expr(stmt::Expr)) || isa(stmt, LineNumberNode) || stmt === nothing,
            ...
            local line::Int = method.line
            if isa(stmts[1], LineNumberNode)
                line = shift!(stmts).line
            end
            unshift!(stmts, Expr(:meta, :push_loc, method.file, method.name, line))
...
function meta_elim_pass!(code::Array{Any,1}, propagate_inbounds::Bool)
