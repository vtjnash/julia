...
let exename = `$(Base.julia_cmd()) --precompiled=yes`
    ...
    if Libdl.dlopen_e(splitext(bytestring(Base.JLOptions().image_file))[1]) != C_NULL
    ...
    let code = """
        ...
        let out  = Pipe(),
            ...
            output_correct = (@async @test readchomp(out) == "WARNING: Foo.Deprecated is deprecated.\n  likely near no file:5")
            close(out.in)
            wait(proc)
            @test success(proc)
            @test output_correct
        ...
        let out  = Pipe(),
            ...
            output_correct = @async @test isempty(readstring(out))
    ...
    if VersionNumber(Base.libllvm_version) > v"3.3"
        testdir = mktempdir()
        cd(testdir) do
            rm(testdir)
            @test success(`$exename -e "exit(0)"`)
