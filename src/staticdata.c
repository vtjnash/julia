...
static uintptr_t **sysimg_gvars = NULL;
...
static void jl_load_sysimg_so(void)
{
    ...
    if (!imaging_mode && jl_options.use_precompiled==JL_OPTIONS_USE_PRECOMPILED_YES) {
        sysimg_gvars = (uintptr_t**)jl_dlsym(jl_sysimg_handle, "jl_sysimg_gvars");
...
static void jl_write_values(jl_serializer_state *s)
{
    ...
    for (i = 0, len = backref_table_numel * 2; i < len; i += 2) {
        ...
        write_padding(s->s, LLT_ALIGN(ios_pos(s->s) + 8, 16) - ios_pos(s->s) - 8);
        // write header
...
{
    uintptr_t base = (uintptr_t)&s->s->buf[0];
    uintptr_t size = (uintptr_t)&s->s->size;
    ...
    for (i = 0; i < sysimg_fvars_max; i++) {
        ...
        if (offset != 0) {
            ...
            if (fvars == NULL) {
                ...
                linfos[i] = li;
                jl_fptr_to_llvm((jl_fptr_t)(uintptr_t)fvars[i], li, cfunc);
            }
        }
    }
    if (fvars == NULL) {
...
JL_DLLEXPORT void jl_restore_system_image(const char *fname)
{
    ...
    if (jl_sysimg_handle) {
        ...
        JL_SIGATOMIC_BEGIN();
        ios_seek_end(&f);
        size_t len = ios_pos(&f) + 1;
        ios_seek(&f, 0);
        if (ios_readprep(&f, len) != len - 1)
            jl_errorf("Error reading system image file.");
        char *sysimg = ios_take_buffer(&f, &len);
        ios_close(&f);
