...
static GlobalValue *stringConst(const std::string &txt)
{
    static int strno = 0;
    std::stringstream ssno;
    ssno << "_j_str" << strno++;
    GlobalVariable *gv = new GlobalVariable(*jl_builderModule,
...
//        vname += "_j_str";
//        vname += ssno.str();
//        gv = new GlobalVariable(*jl_builderModule,
//                                T_int8,
//                                true,
//                                GlobalVariable::ExternalLinkage,
//                                NULL,
//                                vname);
//        stringConstants[txt] = global_proto(gv);
//        jl_sym_t *sym = jl_symbol(txt.c_str());
//        *(jl_sym_t**)jl_emit_and_add_to_shadow(gv, sym) = sym;
//        strno++;
//    }
//    else {
//        gv = prepare_global(gv);
//    }
//    return builder.CreateConstInBoundsGEP1_32(T_int8,
//            builder.CreateLoad(T_pint8, ConstantExpr::getBitCast(gv, T_ppint8)),
//            sizeof(jl_sym_t));
}

static Value *GetStringPtr(llvm::IRBuilder <> *builder, GlobalValue *GV, StringRef Name)
{
...
static void just_emit_error(const std::string &txt, jl_codectx_t *ctx)
{
    builder.CreateCall(prepare_call(jlerror_func),
            GetStringPtr(&builder, stringConst(txt), "just_emit_error"));
}
...
static void emit_type_error(const jl_cgval_t &x, jl_value_t *type, const std::string &msg,
                            jl_codectx_t *ctx)
{
    Value *fname_val = GetStringPtr(&builder, stringConst(ctx->funcName), "fname_val");
    Value *msg_val = GetStringPtr(&builder, stringConst(msg), "msg_val");
#ifdef LLVM37
