...
static jl_lambda_info_t *jl_add_static_parameters(jl_ast_info_t *l, jl_svec_t *sp, jl_tupletype_t *types)
{
    JL_GC_PUSH1(&sp);
...
static jl_lambda_info_t *cache_method(jl_methtable_t *mt, jl_tupletype_t *type,
                                      jl_ast_info_t *method, jl_tupletype_t *decl,
...
JL_DLLEXPORT jl_ast_info_t *jl_instantiate_staged(jl_ast_info_t *generator, jl_tupletype_t *tt, jl_svec_t *env)
{
    jl_expr_t *ex = NULL;
    ...
    jl_ast_info_t *func = (jl_ast_info_t*)jl_toplevel_eval_in_warn(generator->module, (jl_value_t*)ex, 1);
                                     ...
                                     int check_amb, int8_t isstaged, jl_value_t *parent)
{
    ...
    while (l != (void*)jl_nothing) {
        if (((l->tvars==jl_emptysvec) == (tvars==jl_emptysvec)) &&
            ...
            if (check_amb && l->func && method) {
                jl_module_t *newmod = (jl_is_ast_info(method) ? ((jl_ast_info_t*)method)->module : ((jl_lambda_info_t*)method)->def->module);
                jl_module_t *oldmod = (jl_is_ast_info(l->func) ? ((jl_ast_info_t*)l->func)->module : ((jl_lambda_info_t*)l->func)->def->module);
    ...
    while (l != (void*)jl_nothing) {
        if (jl_args_morespecific((jl_value_t*)type, (jl_value_t*)l->sig))
            break;
        if (check_amb) {
            jl_sym_t *name = (jl_is_ast_info(method) ? ((jl_ast_info_t*)method)->name : ((jl_lambda_info_t*)method)->def->name);
...
jl_methlist_t *jl_method_table_insert(jl_methtable_t *mt, jl_tupletype_t *type,
                                      jl_ast_info_t *method, jl_svec_t *tvars,
                                      int8_t isstaged)
{
...
static void _compile_all_deq(jl_array_t *found)
{
    ...
    for (found_i = 0; found_i < found_l; found_i++) {
        ...
        if (jl_is_ast_info(meth->func)) {
            // method definition -- compile via unspecialized field
            jl_ast_info_t *func = (jl_ast_info_t*)meth->func;
...
static void _compile_all_enq_ml(jl_methlist_t *ml, jl_array_t *found)
{
    while (ml != NULL && (jl_value_t*)ml != jl_nothing) {
        if (ml->func != NULL && !ml->isstaged) {
            ...
            jl_lambda_info_t *linfo;
            if (jl_is_ast_info(ml->func)) {
                // method definition -- compile via unspecialized field
                jl_ast_info_t *func = (jl_ast_info_t*)ml->func;
                linfo = func->unspecialized;
                if (linfo == NULL) {
...
jl_value_t *jl_gf_invoke(jl_tupletype_t *types0, jl_value_t **args, size_t nargs)
{
    ...
    if (mfunc != NULL) {
        ...
        jl_ast_info_t *func = (jl_ast_info_t*)m->func;
        if (m->isstaged)
...
void print_func_loc(JL_STREAM *s, jl_value_t *li)
{
    jl_ast_info_t *ast = (jl_is_ast_info(li) ? (jl_ast_info_t*)li : ((jl_lambda_info_t*)li)->def);
    long lno = ast->line;
    if (lno > 0) {
...
void jl_add_method(jl_function_t *gf, jl_tupletype_t *types, jl_ast_info_t *meth,
                   jl_svec_t *tvars, int8_t isstaged)
{
