...
static void jl_serialize_value_(ios_t *s, jl_value_t *v)
{
    ...
    else if (jl_is_ast_info(v)) {
        writetag(s, jl_ast_info_type);
        jl_ast_info_t *li = (jl_ast_info_t*)v;
...
static void jl_serialize_methtable_from_mod(ios_t *s, jl_methtable_t *mt, int8_t iskw)
{
    ...
    while (ml != (void*)jl_nothing) {
        if (module_in_worklist(((jl_ast_info_t*)ml->func)->module)) {
...
static void remove_specializations_from_replaced_modules(jl_methlist_t *l)
{
    while (l != (void*)jl_nothing) {
        assert(jl_is_ast_info(l->func));
        jl_array_t *a = ((jl_ast_info_t*)l->func)->specializations;
        if (a) {
    ...
    while (l != (void*)jl_nothing) {
        if (type_has_replaced_module((jl_value_t*)l->sig))
            *pl = l->next;
            ...
            pl = &l->next;
        else if (is_module_replaced((jl_is_ast_info(l->func) ? (jl_ast_info_t*)l->func : ((jl_lambda_info_t*)l->func)->def)->module))
            *pl = l->next;
        else
            pl = &l->next;
        l = l->next;
...
static jl_value_t *jl_deserialize_value_(ios_t *s, jl_value_t *vtag, jl_value_t **loc)
{
    ...
    if (vtag == (jl_value_t*)jl_simplevector_type ||
    ...
    else if (vtag == (jl_value_t*)jl_ast_info_type) {
        jl_ast_info_t *li =
        ...
        li->def = (jl_ast_info_t*)jl_deserialize_value(s, (jl_value_t**)&li->def);
        jl_gc_wb(li, li->def);
        li->unspecialized = (jl_lambda_info_t*)jl_deserialize_value(s, (jl_value_t**)&li->unspecialized);
        if (li->unspecialized) jl_gc_wb(li, li->unspecialized);
...
static void jl_deserialize_lambdas_from_mod(ios_t *s)
{
    while (1) {
        ...
        jl_ast_info_t *meth = (jl_ast_info_t*)jl_deserialize_value(s, NULL);
...
JL_DLLEXPORT jl_value_t *jl_compress_ast(jl_ast_info_t *li, jl_value_t *ast)
{
    JL_SIGATOMIC_BEGIN();
    JL_LOCK(dump); // Might GC
    DUMP_MODES last_mode = mode;
...
JL_DLLEXPORT jl_value_t *jl_uncompress_ast(jl_ast_info_t *li, jl_value_t *data)
{
    JL_SIGATOMIC_BEGIN();
    JL_LOCK(dump); // Might GC
...
void jl_init_serializer(void)
{
    ...
    void *tags[] = { jl_symbol_type, jl_gensym_type, jl_datatype_type,
                     ...
                     jl_module_type, jl_tvar_type, jl_lambda_info_type, jl_ast_info_type,
                     ...
                     jl_ast_info_type->name, jl_module_type->name, jl_function_type->name,
