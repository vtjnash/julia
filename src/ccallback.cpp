...
//make_ccallback(f::Function, rt::Any, tt::Tuple, [call_conv::Symbol])
static Value *emit_ccallback(jl_value_t **args, size_t nargs, jl_codectx_t *prevctx) {
//extern "C" DLLEXPORT void *ccallback(jl_function_t *f, jl_value_t *rt, jl_tuple_t *tt, jl_sym_t *call_conv) {
    ...
    f = emit_expr()
			jl_interpret_toplevel_expr_in(ctx->module, args[1],
                                      &jl_tupleref(ctx->sp,0),
                                      jl_tuple_len(ctx->sp)/2);
    JL_TYPECHK(make_callback, function, f);
