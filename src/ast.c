...
static jl_value_t *scm_to_julia_(fl_context_t *fl_ctx, value_t e, int eo)
{
    ...
    if (iscons(e) || e == fl_ctx->NIL) {
        ...
        if (sym == lambda_sym) {
            ...
            nli = jl_new_ast_info((jl_value_t*)ex, tvars, jl_current_module);
...
jl_ast_info_t *jl_wrap_expr(jl_value_t *expr)
{
    ...
    jl_ast_info_t *li = jl_new_ast_info((jl_value_t*)le, jl_emptysvec, jl_current_module);
    JL_GC_POP();
    return li;
}
...
{
    if (jl_is_symbol(expr) || jl_is_ast_info(expr)) {
...
jl_value_t *jl_preresolve_globals(jl_value_t *expr, jl_ast_info_t *lam)
{
    ...
    else if (jl_is_ast_info(expr)) {
        jl_ast_info_t *l = (jl_ast_info_t*)expr;
