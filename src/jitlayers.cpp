...
//    for (Module::named_metadata_iterator I = src->named_metadata_begin(), E = src->named_metadata_end(); I != E;) {
//        NamedMDNode *sG = &*I;
//        NamedMDNode *dG = dest->getOrInsertNamedMetadata(sG->getName());
//        ++I;
//        for (NamedMDNode::op_iterator OpI = sG->op_begin(), OpE = sG->op_end(); OpI != OpE; ++OpI) {
//            dG->addOperand(*OpI);
//        }
//        sG->eraseFromParent();
//    }
...
static void jl_finalize_function(Function *F, Module *collector = NULL)
{
    ...
    if (m) {
        std::vector<Function*> tofinalize;
        for (Module::iterator I = m->begin(), E = m->end(); I != E; ++I) {
            ...
            if (!F->isDeclaration()) {
                ...
                tofinalize.push_back(F);
            }
        }
        for (std::vector<Function*>::iterator I = tofinalize.begin(), E = tofinalize.end(); I != E; ++I) {
            jl_finalize_function(*I, collector ? collector : m.get());
        }
        if (collector) {
...
// Connect Modules via prototypes
...
{
    PointerType *T = cast<PointerType>(gv->getType()->getElementType()); // pointer is the only supported type currently
