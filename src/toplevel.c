...
jl_value_t *jl_toplevel_eval_flex(jl_value_t *e, int fast)
{
    ...
    if (head == thunk_sym) {
        thk = (jl_ast_info_t*)jl_exprarg(ex,0);
        assert(jl_is_ast_info(thk));
...
void jl_check_static_parameter_conflicts(jl_ast_info_t *li, jl_svec_t *t, jl_sym_t *fname)
{
    jl_array_t *vinfo;
    size_t nvars;
...
static jl_ast_info_t *expr_to_lambda_ast(jl_ast_info_t *f)
{
    ...
    // wrap in a ASTInfo
    f = jl_new_ast_info((jl_value_t*)f, tvar_syms, jl_current_module);
