...
typedef struct _jl_lambda_info_t {
    ...
    // a slower-but-works version of this function as a fallback
    struct _jl_lambda_info_t *unspecialized;
    jl_ast_info_t *def;  // original this is specialized from
    uint8_t pure;
...
typedef struct _jl_methlist_t {
    ...
    int8_t isstaged;
    jl_svec_t *tvars;
    jl_value_t *func; // TODO: jl_ast_info_t, but currently this is used for cachelist which has jl_lambda_info_t
...
JL_DLLEXPORT jl_lambda_info_t *jl_new_lambda_info(jl_ast_info_t *ast,
                                                  jl_svec_t *sparam_vals,
                                                  jl_datatype_t *specTypes);
JL_DLLEXPORT jl_ast_info_t *jl_new_ast_info(jl_value_t *ast,
                                            jl_svec_t *tvars,
                                            jl_module_t *ctx);
...
JL_DLLEXPORT void jl_method_def(jl_svec_t *argdata, jl_ast_info_t *f, jl_value_t *isstaged);
...
JL_DLLEXPORT jl_value_t *jl_prepare_ast(jl_ast_info_t *li);
