...
#if LLVM_VERSION_MAJOR == 3 && LLVM_VERSION_MINOR == 3
#define SymbolicFile ObjectFile
#define createUniqueFile unique_file
#define getError() operator error_code()
#endif
...
using namespace llvm39;

NewArchiveMember::NewArchiveMember(std::unique_ptr<MemoryBuffer> BufRef)
    : Buf(std::move(BufRef)) {}
...
writeSymbolTable(raw_fd_ostream &Out, object::Archive::Kind Kind,
  ...
  for (unsigned MemberNum = 0, N = Members.size(); MemberNum < N; ++MemberNum) {
    MemoryBuffer &MemberBuffer = *Members[MemberNum].Buf.get();
#if LLVM_VERSION_MAJOR == 3 && LLVM_VERSION_MINOR == 3
    Expected<std::unique_ptr<object::ObjectFile>> ObjOrErr =
        std::unique_ptr<object::ObjectFile>(object::ObjectFile::createObjectFile(&MemberBuffer));
#else
    Expected<std::unique_ptr<object::SymbolicFile>> ObjOrErr =
        object::SymbolicFile::createSymbolicFile(
            MemberBuffer.getMemBufferRef(), sys::fs::file_magic::unknown, &Context);
    if (!ObjOrErr) {
...
#endif
    object::SymbolicFile &Obj = **ObjOrErr;
...
#if LLVM_VERSION_MAJOR == 3 && LLVM_VERSION_MINOR == 3
    error_code itererr;
    for (object::symbol_iterator sym_iter = Obj.begin_symbols(),
            sym_end = Obj.end_symbols();
            sym_iter != sym_end;
            sym_iter.increment(itererr)) {
      const object::SymbolRef &S = *sym_iter;
      uint32_t Symflags;
      if (S.getFlags(Symflags))
        continue;
#else
...
llvm39::writeArchive(StringRef ArcName,
  ...
  tool_output_file Output(TmpArchive.c_str(), ErrorInfo, TmpArchiveFD);
#else
  ...
  std::vector<MemoryBuffer*> Members;
  ...
  if (WriteSymtab) {
    ...
    if (auto EC = MemberReferenceOffsetOrErr.getError())
      return std::make_pair(ArcName, EC);
    MemberReferenceOffset = *MemberReferenceOffsetOrErr;
  }
  ...
  sys::fs::rename(TmpArchive.c_str(), ArcName);
